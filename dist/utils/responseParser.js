...
export function parseComponentResponse(response) {
    ...
    sections.forEach(section => {
        if (section.includes('VISUAL COMPONENT HIERARCHY:') || section.includes('COMPONENTS NEEDED:')) {
            ...
            componentBlocks.slice(1).forEach(block => {
                ...
                if (component)
                    componentsNeeded.push(component);
            });
        }
            ...
            relationshipBlocks.slice(1).forEach(block => {
                ...
                if (relationship)
                    visualRelationships.push(relationship);
            });
        }
...
function parseUIComponentBlock(block) {
...
function parseVisualRelationshipBlock(block) {
    const lines = block.split('\n').map(line => line.trim());
    const relationship = {};
    lines.forEach(line => {
...
function parseShadcnIntegrationSection(section) {
...
function extractTextContent(section, marker) {
    const lines = section.split('\n');
...
function extractSubListItems(lines, startIndex) {
    const items = [];
