...
export class V0McpServer {
  ...
  private setupTools(): void {
    ...
    this.server.addTool({
      ...
      execute: async (args) => {
        try {
          ...
          const exports = [];
          if (parsed.code.includes('export default')) exports.push('default');
          ...
          const componentResponse: ComponentGenerationResponse = {
    ...
    this.server.addTool({
      ...
      execute: async (args) => {
        try {
          const multimodalRequest: MultimodalRequest = {
